...
def generateSurveyData():
    ...
    try:
        # Newline argument is required for later Python versions,
        # otherwise a gap is inserted between each row
...
def getSurveyResponse():
    ...
    return (cinema, regularCinema, day, sessionConvenience, travelTime,
            movieFrequency, goldClass, overallCleanliness, screenSize, pictureQuality,
            soundQuality, seatQuality, temperatureQuality, staffLevel, staffQuality,
            bathroomCleanliness, vrewardsMember, purchaseFoodDrinks, foodSelection,
            foodQuality, foodPrice, wouldRecommend, giftUser, receiveOffers, username, email)

def getSurveyQuestions():
    surveyQuestions = (
        'Which village cinema did you last visit to watch a movie?',
        ...
        'Approximately how long did it take for you to reach the cinema from home '+
        'on your last visit?',
        'Including your most recent visit, how frequently have you been to the movies '+
        ...
        'Based on your last experience, how likely would you recommend the Village cinema'+
        ' you visited to a friend or colleague?',
    ...
    # Future question for free text: 'Is there anything else you would
    # like to share about your cinema experience?'
...
def getLikert(scaleType="Traditional"):
    ...
    if scaleType == "Convenience":
        scaleDescriptions = ('Very Inconvenient', 'Inconvient', 'Indifferent', 'Convenient',
                             'Very Convenient')
    elif scaleType == "Temperature":
        scaleDescriptions = ('Way too cold', 'A little cool', 'Just right', 'A little warm',
                             'Way too hot')
    else:
        scaleDescriptions = ('Strongly disagree', 'Disagree', 'Neither agree nor disagree',
...
def getVariableData(dataType="Unknown"):
    ...
    if dataType == "Travel Time":
        variableDescriptions = ('Less than 10 minutes', 'Less than 30 minutes',
...
def getCinema():
    cinemaLocations = ('Airport West', 'Albury', 'Bendigo', 'Century City', 'Coburg Drive-In',
                       'Crown', 'Doncaster', 'Eastlands', 'Fountain Gate', 'Geelong', 'Glenorchy',
                       'Hobart', 'Jam Factory', 'Karingal', 'Knox', 'Launceston', 'Morwell',
                       'Plenty Valley', 'Rivoli', 'Rosebud', 'Shepparton', 'Sorrento', 'Southland',
                       'Sunshine', 'Warragul', 'Warnambool', 'Werribee')

    return cinemaLocations[randint(0, len(cinemaLocations) - 1)]

def getGoldClass(cinema):
    goldClassLocations = ('Albury', 'Bendigo', 'Century City', 'Crown', 'Doncaster',
                          'Fountain Gate', 'Geelong', 'Hobart', 'Jam Factory', 'Karingal', 'Knox',
                          'Plenty Valley', 'Rivoli', 'Southland', 'Sunshine', 'Werribee')
